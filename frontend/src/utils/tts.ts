...
export const speak = (text: string, lang = 'vi-VN'): void => {
  ...
  if (allVoices.length === 0) {
    ...
    // Nếu vẫn không có, dùng giọng mặc định của trình duyệt
    if (!selectedVoice) {
      selectedVoice = allVoices.find(voice => voice.default);
    }
    
    console.log('[TTS] Selected voice:', selectedVoice ? selectedVoice.name : 'None found, using browser default.');

    if (selectedVoice) {
      utterance.voice = selectedVoice;
      utterance.lang = selectedVoice.lang;
    } else {
      // Fallback if no suitable voice is found
      utterance.lang = lang;
    }
  }
