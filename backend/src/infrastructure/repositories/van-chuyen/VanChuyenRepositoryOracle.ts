...
import { GetE1DetailsResponseDto, E1DetailInfo, E1BD10Info } from '../../../application/dto/van-chuyen/GetE1DetailsResponseDto';
import { getConnectionByMaBC } from '../../../config/database/db.config';
import { E1Info } from '../../../application/dto/van-chuyen/SearchE1ResponseDto';
    ...
    try {
      ...
      const e1Details: E1DetailInfo[] = e1Rows.map((row: any) => {
        ...
        return {
          ...
          thongTinCT: row.THONGTINCT,
        };
      });
      
      // Tạm thời quay lại trả về mảng rỗng cho bd10Details theo yêu cầu, chờ thống nhất phương án "Hợp nhất thông minh".
      const bd10Details: E1BD10Info[] = [];
      // const bd10Details: E1BD10Info[] = bd10Rows.map((row: any) => ({
      //   stt: row.STT,
      //   ngayBD10: row.NGAYBD10 ? format(new Date(row.NGAYBD10), 'dd/MM/yyyy') : '',
      //   ngayXacNhanDi: row.NGAYXACNHANDI ? format(new Date(row.NGAYXACNHANDI), 'dd/MM/yyyy') : '',
      //   buuCucGiao: row.BUUCUCGIAO,
      //   buuCucNhan: row.BUUCUCNHAN,
      //   lanLapMaBD10: row.LANLAP_MABD10,
      // }));
      ...
      // Ném lỗi để lớp trên (use case, controller) có thể bắt và trả về lỗi 500
      throw new Error(err.message || 'Có lỗi xảy ra ở phía server khi lấy chi tiết E1.');
    } finally {
      if (conn) await conn.close();
    }
  }
